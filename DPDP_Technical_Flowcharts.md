...
flowchart TD
    ...
    subgraph DATA_LAYER [Knowledge Data Layer]
        ...
        N[("wiki/act and rules\nand concepts .md files")]
...
flowchart TD
    ...
    subgraph INGEST_ACT ["ingest_act — Act Ingestion"]
        ...
        A7["Write wiki/act/section_N.md\nYAML frontmatter\n+ Summary + Key Points\n+ Verbatim + Cross-Refs"]
        A1 --> A2 --> A3
        A3 --> A4
        A3 --> A5
        A3 --> A6
    ...
    subgraph INGEST_RULES ["ingest_rules — Rules Ingestion"]
        ...
        B6["_rule_key_points\nextract sub-rule 1 2 3 items"]
        B7["Write wiki/rules/rule_N.md"]
...
> **Problem Solved:** The index must always reflect the current wiki state. If a wiki page changes, embeddings must be regenerated — but not unnecessarily.
...
flowchart TD
    ...
    Q1 -- No --> REBUILD
    Q1 -- Yes --> Q2{"col.count == 0?"}
    Q2 -- Yes --> REBUILD
    Q2 -- No --> Q3["Get db_sqlite\nmtime timestamp\nfrom chroma.sqlite3"]
    Q3 --> LOOP["For each .md file in wiki"]
    LOOP --> Q4{"file.mtime\ngreater than db_mtime?"}
...
## 4. RAG Query Pipeline — Full 5-Step Detail

> **Core Architecture:** Wiki-Page RAG (current production system). The 5-step pipeline that runs on every user query.
...
flowchart TD
    ...
    subgraph S1 ["STEP 1 — Semantic Retrieval"]
        ...
        R5["Deduplicate by page_id\nkeep highest score\nif same page from multiple queries"]
    ...
    SC2 & SC4 & SC7 --> OUT(["Streamed Answer Token by Token\nCited with Act section and Rules numbers"])
...
> The earlier implementation. Understand this because interviewers will ask about architectural evolution.
...
> How the LLM self-classifies and adapts reasoning depth to the complexity of the question.
...
flowchart TD
    ...
    subgraph POOL [Key Pool State — Circular]
        K1["Key 1\nIndex 0"]
        K2["Key 2\nIndex 1"]
        K3["Key 3\nIndex 2"]
        K4["Key 4\nIndex 3"]
...
```mermaid
flowchart TD
    START(["LLM starts generating\nfirst tokens"]) --> BUF["Buffer incoming tokens\nbuffer = empty string"]
    ...
    C1 -- No --> C2{"READ in buffer?"}

    C2 -- Yes --> BRACKET{"Has closing\nbracket?"}
    BRACKET -- "Not yet and\nbuffer less than 300 chars" --> TOK
...
```mermaid
flowchart TD
    subgraph TURNS ["Rolling 4-Turn Window"]
        U1["Turn 1 User:\nraw question"]
        A1["Turn 1 LLM:\nfull answer up to 4096 tokens"]
        U2["Turn 2 User:\nfollow-up question"]
        A2["Turn 2 LLM:\nanswer 2"]
        U1 --> A1 --> U2 --> A2
    end

    subgraph HISTORY ["What Gets Stored in history list"]
        H1["role: user\ncontent: core_inquiry truncated to 200 chars"]
        H2["role: assistant\ncontent: full_response truncated to 500 chars"]
        H3["role: user\ncontent: next core_inquiry"]
        H4["role: assistant\ncontent: next response truncated"]
        H1 --> H2 --> H3 --> H4
    end

    subgraph MSG_BUILD ["Messages Sent to LLM on Each Turn"]
        M1["1. system: SYSTEM_PROMPT"]
        M2["2. history entries\nmax 8 messages total\n4 user + 4 assistant"]
        M3["3. user: wiki pages context block\n+ current query + query metadata"]
        M1 --> M2 --> M3
    end
    ...
    subgraph WHY ["Design Rationale"]
        ...
        W4["4 turns sufficient for follow-ups\nwhile keeping total prompt tokens bounded"]
    end
...
mindmap
  root((DPDP Platform\nTech Stack))
    LLM Layer
      Gemini 2.5 Flash Answer Generator
    ...
    Embedding Model
      BAAI bge-small-en-v1.5
        MTEB retrieval score 51.7
        Beats MiniLM-L6 score 40.1
        Asymmetric retrieval training
        Superior on legal text
    ...
    Vector Database
      ChromaDB
        ...
        Python native
        Auto HNSW index
        Pinecone rejected: cost
        Weaviate rejected: complexity
    Backend Framework
      FastAPI planned
      ...
      requests library current
        ...
        SSE stream parsing control
    Data Formats
      Structured JSON
        dpdp_titled.json hierarchical Act
        newtry.json compliance tree
        Machine readable
        Enables multi-tier indexing
      Markdown Wiki Pages
        YAML frontmatter metadata
        Human and machine readable
        Git versionable
        mtime freshness detection
    NLP and Processing
      pdfplumber
        Gazette PDF extraction
...
flowchart TD
    ...
    subgraph L1 ["Layer 1 — Grounding Instruction in System Prompt"]
    ...
    subgraph L4 ["Layer 4 — Self-Correction READ Loop"]
        ...
        R2["System fetches requested pages\nRe-invokes LLM with complete context"]
    ...
    subgraph L5 ["Layer 5 — Ambiguity Resolution Rule"]
        ...
        A2["Cite provision\nand answer directly"]
        A3["Present BOTH interpretations\nwith operational implications"]
...
## 12. Architectural Evolution: 3-Tier to Wiki-Page RAG
...
flowchart LR
    ...
    subgraph NEW ["Wiki-Page RAG — core/vectorstore.py"]
        ...
        N4["READ self-correction loop\nreplaces cross-ref expansion"]
        N5["Fewer richer documents\nto LLM context"]
        N1 --> N2 --> N3 --> N4 --> N5
    end

    subgraph TRADEOFFS ["Migration Tradeoffs"]
        T1["GAINED:\nRicher per-doc context\nSimpler retrieval logic\nBetter embedding model\nHuman-readable wiki\nGit-versionable knowledge"]
        T2["COST:\nLess sub-clause granularity\nREAD loop adds latency\non context misses\nIngest step required"]
    end
...
sequenceDiagram
    ...
    participant ENC as "Encoder gemini-2.0-flash"
    participant VEC as "ChromaDB WikiVectorStore"
    participant CTX as Context Builder
    participant ANS as "Answer LLM gemini-2.5-flash"
    participant WIKI as "Wiki Pages In-Memory Cache"

    U->>FE: Can a hospital use treatment data to train an AI model?
    FE->>ENC: POST /api/chat with query

    Note over ENC: temp=0.0, max_tokens=300
    ENC-->>FE: query_type=situation, actors=hospital+patients, search_queries = purpose limitation consent, legitimate uses section 7, research archiving rule 16

    FE->>VEC: query purpose limitation consent new use top_k=10
    VEC-->>FE: section_6 score=0.88, section_7 score=0.85, consent score=0.82
    FE->>VEC: query legitimate uses without consent section 7 top_k=10
    VEC-->>FE: section_7 score=0.91, section_4 score=0.79
    FE->>VEC: query research archiving exemption rule 16 top_k=10
    VEC-->>FE: rule_16 score=0.87, section_17 score=0.74

    Note over FE: Dedup + sort by score, take top 10 pages
    ...
    FE->>CTX: build_context_block with all pages
    CTX-->>FE: Numbered context string with source labels and scores

    FE->>ANS: POST streamGenerateContent with SYSTEM_PROMPT + history + context

    Note over ANS: Buffer first 150 tokens

    ANS-->>FE: token: think-tag detected
    Note over FE: think tag detected, context sufficient, start streaming

    ANS-->>FE: TIER 3 reasoning blocks inside think tags, then final answer

    ANS-->>FE: Fresh consent is not automatically required but conditions apply...

    Note over ANS: No READ flag triggered, all relevant pages retrieved

    FE->>U: Stream tokens to browser via SSE
    U->>U: Sees answer building word by word

    Note over FE: Update history: user=core_inquiry 200 chars, assistant=answer 500 chars
```
...
|-----------|-----------|-------------------|
| **RAG** | Main chatbot pipeline | Grounds LLM to actual legal text; prevents hallucination of provisions |
...
| **YAML Frontmatter** | Wiki pages | Machine-readable metadata (source, tags, cross-refs) in human-readable files |
...
| **Regex Cross-Ref Extraction** | `_act_refs()`, `_rule_refs()` | Detects Section N patterns to build the knowledge graph |
...
| **Token Buffering** | `_buffered_stream()` | Detects think-tag vs READ flag in first 150 tokens before passing through |
| **SSE Streaming** | gemini_stream_iter() | Token-by-token delivery — responsive UX, no wait for full response |
| **Round-Robin Key Pool** | `_next_key()` | ~5x free-tier quota; automatic 429 failover across 5 keys |
...
| **Ambiguity Resolution Rule** | SYSTEM_PROMPT | Prevents arbitrary legal interpretation; presents both readings on genuine conflict |
| **Self-Correction Loop** | ask_wiki_llm() | LLM signals context gaps; system fetches and re-invokes transparently |
